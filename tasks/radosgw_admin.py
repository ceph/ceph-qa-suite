...
from tasks.util.compat import StringIO, range, str
...
def task(ctx, config):
    ...
    if multi_region_run:
        ...
        for agent_client, c_config in ctx.radosgw_agent.config.items():
            ...
            new_data['mtime'] = str(mtime.strftime("%Y-%m-%d %H:%M:%S.%fZ"))
        ...
        for agent_client, c_config in ctx.radosgw_agent.config.items():
            ...
            for full in (True, False):
                ...
                assert dest_k is None, 'object not deleted from destination zone'
        ...
        for agent_client, c_config in ctx.radosgw_agent.config.items():
            ...
            for full in (True, False):
                ...
                for i in range(20):
                    ...
                    assert dest_k is None, 'object %d not deleted from destination zone' % i
        bucket.delete()
    ...
    for obj in out:
        ...
        assert rgwlog['bucket_owner'] == user1 or rgwlog['bucket'] == bucket_name + '5' or rgwlog[
                                                                                               'bucket'] == bucket_name2
    ...
    while time.time() - timestamp <= (20 * 60):  # wait up to 20 minutes
        (err, out) = rgwadmin(ctx, client, ['usage', 'show', '--categories',
                                            'delete_obj'])  # last operation we did is delete obj, wait for it to flush
