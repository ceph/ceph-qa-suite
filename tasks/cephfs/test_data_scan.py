...
class TestDataScan(CephFSTestCase):
    ...
    def test_scan_links(self):
        ...
        out_file = "/tmp/{0}_{1}".format(dirfrag1_oid, file1_key);
        self.fs.rados(["getomapval", dirfrag1_oid, file1_key, out_file])
        with open(out_file, mode='rb') as file:
            omap_file1_value = file.read()
            self.fs.rados(["setomapval", dirfrag2_oid, file1_key], stdin_data=omap_file1_value)
        self.assertIn(file1_key, self._dirfrag_keys(dirfrag2_oid))

        # remote a remote link, make inode link count incorrect
