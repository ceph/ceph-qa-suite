"""
  ...
  It requires Ceph 2 clients which should be configured to have access to same rbd pools
"""
import contextlib
import logging
...
def task(ctx, config):
    """
     ...
     Needs 2 ceph clients which should be configured to have access to same rbd pools
     Most of the config items are configurable to include what image to be run
     the vm domain definition and the livirt secret uuid.

     qemu-live-migration:
        libvirtd-conf: http://ceph/qemu-libvirtd.conf (this uses tcp protool for migration)
        libvirtd-args: http://ceph/qemu-libvirtd-args.conf ( should set the listen option)
        qemu-image: http://cloud-images.ubuntu.com/releases/14.04/release/ubuntu-14.04-server-cloudimg-amd64-disk1.img 
        secret-file: http://ceph/qemu-secret-file.txt
        vm-file:  http://ceph/qemu-vm-test.xml

    """
    clients = ctx.cluster.only(teuthology.is_type('client'))  
    assert len(clients.remotes) == 2, "Needs 2 clients to run qemu live migration test"
...
def get_test_files(config):
    ...
    RBD_IMAGE_NAME which is replace with real rbd image name thats created
    and
...
def ceph_auth_create(config):
    ...
    client = config.get('client0')
    out=StringIO.StringIO()
    client.run(args=['sudo', 'ceph', 'auth', 'add', 'client.libvirt', run.Raw("mon 'allow *' osd 'allow *'")])
...
def libvirt_set_secret(config):
    ...
    for client in clients:
       ...
       client.run(args=['cd', qemu_test_dir, run.Raw(';'), 'sudo', 'virsh', 'secret-define',
                        run.Raw('--file'), 'secret.xml'],stdout=out)
       uuid=re.search(r'(\w{8}-\w{4}-\w{4}-\w{4}-\w{12})', out.getvalue())
       suuid=uuid.group(1)
       client.run(args=['cd', qemu_test_dir, run.Raw(';'), 'sudo', 'virsh', 'secret-set-value',
                            run.Raw('--secret'), suuid, run.Raw('--base64'), run.Raw("$(cat client.libvirt)")])
...
def create_rbd_image(config):
    ...
    client = config.get('client0')
    client.run(args=['qemu-img', 'convert', run.Raw('-f'), 'qcow2', run.Raw('-O'), 'raw', img_name,
                     rbd_name])
...
def create_and_define_vm(config):
    ...
    client.run(args=['sudo', 'cat', '/etc/ceph/ceph.conf', run.Raw('|'), 
                run.Raw('grep -oE "([0-9]{1,3}\.){3,3}[0-9]{1,3}:6789"')],stdout=out)
...
def cleanup(config):
    ...
    for client in clients:
        client.run(args=['sudo', 'cp', '{qd}/libvirtd.conf'.format(qd=qemu_test_dir),
                            '/etc/libvirt/libvirtd.conf'])
        client.run(args=['sudo', 'cp', '{qd}/libvirtd'.format(qd=qemu_test_dir),
